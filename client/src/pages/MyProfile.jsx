    ...
    <div className="max-w-3xl mx-auto mt-5 p-8 bg-white dark:bg-gray-800 rounded-2xl shadow-lg transition-all">
      ...
      {/* <div className="flex flex-col items-center">
        {isEdit ? (
          <label htmlFor="image" className="cursor-pointer relative group">
            <img
              className="w-36 h-36 rounded-full object-cover ring-4 ring-pink-300 dark:ring-pink-600 opacity-80 group-hover:opacity-100 transition-all"
              src={image ? URL.createObjectURL(image) : studentData.image}
              alt="profile"
            />
            <div className="absolute bottom-2 right-2 bg-pink-600 text-white text-xs px-2 py-1 rounded shadow">
              Change
            </div>
            <input
              onChange={(e) => setImage(e.target.files[0])}
          ...
          <img
            className="w-36 h-36 rounded-full object-cover ring-4 ring-pink-300 dark:ring-pink-600"
            src={studentData.image}
            alt="profile"
          />
        )}
